...
function injectStyles() {
  ...
  s.textContent = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes micPulse{0%,100%{box-shadow:0 0 0 0 rgba(255,0,0,0.4)}70%{box-shadow:0 0 0 8px rgba(255,0,0,0)}}
...
const DARK_THEME = {
  ...
  border:"#2A2A3A", text:"#F0F0FF", textMuted:"#8888AA",
};

const BEIGE_THEME = {
  bg:"#F5F0E8", surface:"#FDFAF5", surface2:"#EDE8DC",
  border:"#D8CFBF", text:"#2C2416", textMuted:"#8C7A60",
};
...
const TRANSLATIONS = {
  ...
  hi: {
    ...
    "Explore Capabilities": "क्षमताएं खोजें",
    "Our Story": "हमारी कहानी",
    "One platform. Every need.": "एक प्लेटफॉर्म। हर जरूरत।",
  ...
  fr: {
    ...
    "View My Recommendations →": "Voir mes recommandations →",
    "Explore Capabilities": "Explorer les capacités",
    "Our Story": "Notre histoire",
    "One platform. Every need.": "Une plateforme. Chaque besoin.",
  ...
  de: {
    ...
    "View My Recommendations →": "Meine Empfehlungen ansehen →",
    "Explore Capabilities": "Fähigkeiten erkunden",
    "Our Story": "Unsere Geschichte",
    "One platform. Every need.": "Eine Plattform. Jeder Bedarf.",
  ...
  ja: {
    ...
    "Solutions": "ソリューション",
    "Get in Touch": "お問い合わせ",
    "View My Recommendations →": "おすすめを見る →",
    "Explore Capabilities": "機能を探る",
    "Our Story": "私たちのストーリー",
    "One platform. Every need.": "一つのプラットフォーム。すべてのニーズ。",
    "What We Do": "私たちの仕事",
  },
  es: {
    "About": "Acerca de",
    ...
    "View My Recommendations →": "Ver mis recomendaciones →",
    "Explore Capabilities": "Explorar capacidades",
    "Our Story": "Nuestra historia",
    "One platform. Every need.": "Una plataforma. Cada necesidad.",
  ...
  pt: {
    ...
    "View My Recommendations →": "Ver minhas recomendações →",
    "Explore Capabilities": "Explorar capacidades",
    "Our Story": "Nossa história",
    "One platform. Every need.": "Uma plataforma. Cada necessidade.",
    "What We Do": "O que fazemos",
  },
  zh: {
    "welcome to your enterprise AI command centre.": "欢迎来到您的企业AI指挥中心。",
     "NexaCore adapts to your industry, region, and team — every insight feels built for you.": "NexaCore适应您的行业、地区和团队——每个洞察都为您量身定制。",
    "Capabilities": "能力",
    "Solutions": "解决方案",
    "Get in Touch": "联系我们",
    "View My Recommendations →": "查看我的推荐 →",
    "Explore Capabilities": "探索能力",
    "Our Story": "我们的故事",
    "One platform. Every need.": "一个平台。每个需求。",
    "What We Do": "我们做什么",
  },
};
function useLang() {
  ...
  const t_ = useCallback((str) => {
    if (lang === "en") return str;
    return TRANSLATIONS[lang]?.[str] || str;
  }, [lang]);

  return { lang, switchLang, t_, translating: false, LANGUAGES };
}
        ...
        {sent ? (
          ...
          <div style={{ width:"100%",maxWidth:520 }}>
            <div className="nc-fade" style={{ marginBottom:32,textAlign:"center" }}>
              <h1 style={{ fontSize:"clamp(1.7rem,4vw,2.4rem)",fontWeight:900,color:t.text,letterSpacing:"-0.8px",marginBottom:10 }}>Let's talk about your enterprise.</h1>
              <p style={{ color:t.textMuted,fontSize:15,lineHeight:1.7 }}>Tell us about your needs. We'll come back with a custom AI deployment plan.</p>
            </div>
            <div style={{ background:t.surface,border:`1px solid ${t.border}`,borderRadius:16,padding:"30px",display:"flex",flexDirection:"column",gap:15 }}>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
                <div><label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Your Name</label><input style={inp} placeholder="Jane Smith" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div>
                <div><label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Work Email</label><input style={inp} type="email" placeholder="jane@company.com" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))}/></div>
              </div>
              <div><label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Company</label><input style={inp} placeholder="Acme Corp" value={form.company} onChange={e=>setForm(p=>({...p,company:e.target.value}))}/></div>
              <div><label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>What are you looking to solve?</label><textarea style={{...inp,resize:"vertical"}} rows={4} value={form.message} onChange={e=>setForm(p=>({...p,message:e.target.value}))}/></div>
              ...
              <p style={{ color:t.textMuted,fontSize:12,textAlign:"center" }}>🔒 We never share your data.</p>
            </div>
          </div>
        )}
      </div>
...
// ─── NAV (with Language + Theme Toggle) ───────────────────────
function Nav({ t, user, onContactClick, lang, switchLang, translating, t_, themeMode, setThemeMode }) {
  const th = COUNTRY_THEMES[user.country]||COUNTRY_THEMES["Other"];
  const [langOpen, setLangOpen] = useState(false);
  const currentLang = LANGUAGES.find(l => l.code === lang) || LANGUAGES[0];

  const themeIcons = { light:"☀️", dark:"🌙", beige:"🌿" };
  const nextTheme  = { light:"dark", dark:"beige", beige:"light" };
    ...
    <nav style={{ position:"sticky",top:0,zIndex:500,background:t.surface+"EE",backdropFilter:"blur(12px)",borderBottom:`1px solid ${t.border}`,padding:"0 32px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
      ...
      <div style={{ display:"flex",gap:26 }}>
        {[["About","#about"],["Capabilities","#capabilities"],["Solutions","#solutions"]].map(([label,href])=>(
          <a key={href} href={href} style={{ color:t.textMuted,textDecoration:"none",fontSize:14,fontWeight:500 }}>{t_(label)}</a>
        ))}
      </div>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
        ...
        {/* Theme toggle */}
        <button
          onClick={() => setThemeMode(m => nextTheme[m])}
          title={`Switch to ${nextTheme[themeMode]} mode`}
          style={{ background:t.surface2,border:`1px solid ${t.border}`,borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:16,transition:"all 0.2s",color:t.text }}
        >
        ...
        <div style={{ position:"relative" }}>
          <button
            ...
            style={{ display:"flex",alignItems:"center",gap:6,background:t.surface2,border:`1px solid ${t.border}`,borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:13,color:t.text,fontWeight:600 }}
          >
            {translating ? "⏳" : currentLang.flag} {currentLang.code.toUpperCase()} ▾
          </button>
          {langOpen && (
            <div style={{ position:"absolute",top:"calc(100% + 6px)",right:0,background:t.surface,border:`1px solid ${t.border}`,borderRadius:10,boxShadow:`0 8px 24px rgba(0,0,0,0.12)`,zIndex:999,minWidth:160,overflow:"hidden" }}>
              {LANGUAGES.map(l => (
                <button key={l.code} onClick={() => { switchLang(l.code); setLangOpen(false); }}
                  style={{ display:"flex",alignItems:"center",gap:10,width:"100%",padding:"9px 14px",background:lang===l.code?t.accentSoft:"transparent",border:"none",cursor:"pointer",fontSize:13,color:lang===l.code?t.accent:t.text,fontWeight:lang===l.code?700:400,textAlign:"left" }}>
                  <span>{l.flag}</span><span>{l.label}</span>
                  {lang===l.code && <span style={{ marginLeft:"auto",color:t.accent }}>✓</span>}
                </button>
              ))}
            </div>
          )}
...
// ─── HERO ─────────────────────────────────────────────────────
function HeroSection({ user, t, t_ }) {
  const th      = COUNTRY_THEMES[user.country]||COUNTRY_THEMES["Other"];
  ...
  return (
    <section style={{ padding:"56px 40px 48px",textAlign:"center",background:t.bg }}>
      ...
      <div style={{ display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap" }}>
        <a href="#solutions" style={{ background:t.accent,color:t.accentText,padding:"12px 26px",borderRadius:10,textDecoration:"none",fontWeight:700,fontSize:14 }}>
          {t_("View My Recommendations →")}
        </a>
      ...
      <div style={{ display:"flex",marginTop:48,justifyContent:"center",borderTop:`1px solid ${t.border}`,borderBottom:`1px solid ${t.border}`,flexWrap:"wrap" }}>
        {[["500+","Enterprise Clients"],["99.9%","Uptime SLA"],["40%","Avg. Efficiency Gain"],["150+","Countries"]].map(([n,l],i,arr)=>(
          <div key={l} style={{ padding:"20px 36px",textAlign:"center",borderRight:i<arr.length-1?`1px solid ${t.border}`:"none" }}>
            <div style={{ fontSize:22,fontWeight:900,color:t.text }}>{n}</div>
    ...
    <section id="about" style={{ padding:"60px 40px",background:t.surface2,borderTop:`1px solid ${t.border}` }}>
      <div style={{ maxWidth:1020,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center" }}>
        <div>
          ...
          <h2 style={{ fontSize:"clamp(1.4rem,2.6vw,2rem)",fontWeight:900,color:t.text,marginBottom:14,lineHeight:1.2 }}>AI that thinks in your industry's language.</h2>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
          {[["2019","Founded"],["500+","Enterprise clients"],["98%","Retention rate"],["48h","Avg. onboarding"]].map(([n,l])=>(
            <div key={l} style={{ background:t.surface,border:`1px solid ${t.border}`,borderRadius:14,padding:"20px",textAlign:"center" }}>
              <div style={{ fontSize:26,fontWeight:900,color:t.accent }}>{n}</div>
    ...
    <section id="capabilities" style={{ padding:"60px 40px",background:t.bg,borderTop:`1px solid ${t.border}` }}>
      <div style={{ maxWidth:1020,margin:"0 auto" }}>
        <div style={{ textAlign:"center",marginBottom:36 }}>
          ...
          <h2 style={{ fontSize:"clamp(1.4rem,2.6vw,2rem)",fontWeight:900,color:t.text,marginBottom:8 }}>One platform. Every need.</h2>
...
function SolutionsSection({ user, t }) {
 ...
 useEffect(()=>{
    setAiProfile(`NexaCore delivers proven ${industry} AI solutions tailored for ${user.country} enterprises.`);
    setProfLoad(false);
  }, [industry, user.country]);

  useEffect(()=>{
    if (!query.trim()) { setAiResult(null); return; }
    // Disabled to preserve API quota - shows all services by default//
    return;
    ...
    <section id="solutions" style={{ padding:"60px 40px",background:t.surface2,borderTop:`1px solid ${t.border}` }}>
      <div style={{ maxWidth:1020,margin:"0 auto" }}>
        ...
        <div style={{ textAlign:"center",marginBottom:28 }}>
          <h2 style={{ fontSize:"clamp(1.4rem,2.6vw,2rem)",fontWeight:900,color:t.text,marginBottom:8 }}>Your personalised AI stack, {user.name}.</h2>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:14 }}>
          {displayed.map((svc,i)=>(
            <div key={svc.title} className="nc-fade" style={{ animationDelay:`${i*0.055}s`,background:t.surface,border:`1.5px solid ${i===0&&!aiResult?t.accent:t.border}`,borderRadius:13,padding:"22px 18px",transition:"transform 0.2s,box-shadow 0.2s" }}
              ...
              <div style={{ width:38,height:38,borderRadius:9,background:t.accentSoft,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,marginBottom:11 }}>{svc.icon}</div>
...
function Chatbot({ user, t }) {
  ...
  useEffect(()=>{
    if (open && msgs.length===0) {
      setMsgs([{ role:"bot", text:`Hi ${user.name}! 👋 I'm the NexaCore assistant. You can type or use the 🎤 mic to speak. Ask me about our ${user.industry} solutions.` }]);
    }
  }, [open]);

  useEffect(()=>{ bottomRef.current?.scrollIntoView({behavior:"smooth"}); }, [msgs]);

  const SYSTEM = `You are NexaCore AI's enterprise assistant. User: Name=${user.name}, Industry=${user.industry}, Country=${user.country}. Reply in 2-4 sentences max. Be professional. Never say you are Claude or Anthropic.`;
  ...
  const send = async (text) => {
    ...
    try {
      ...
      // Read reply aloud if speaking mode
      ...
      {open && (
        <div className="nc-slide" style={{ position:"fixed",bottom:84,right:88,zIndex:900,width:320,maxHeight:490,background:t.surface,borderRadius:16,border:`1px solid ${t.border}`,boxShadow:`0 16px 56px rgba(0,0,0,.18)`,display:"flex",flexDirection:"column",overflow:"hidden" }}>
          ...
          <div style={{ padding:"12px 15px",background:t.accent,display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
            ...
            <button onClick={()=>setSpeaking(s=>!s)} title="Toggle voice readback"
              style={{ background:speaking?"rgba(255,255,255,0.3)":"rgba(255,255,255,0.1)",border:"none",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:14,color:t.accentText }}>
              {speaking ? "🔊" : "🔇"}
            </button>
          </div>
          ...
          {/* Input row */}
          <div style={{ padding:"9px 10px",borderTop:`1px solid ${t.border}`,display:"flex",gap:7,flexShrink:0,alignItems:"center" }}>
            ...
            <button onClick={toggleVoice}
              style={{ background:listening?"#FF3B30":t.surface2,border:`1px solid ${listening?"#FF3B30":t.border}`,borderRadius:8,padding:"8px 10px",cursor:"pointer",fontSize:14,transition:"all 0.2s",animation:listening?"micPulse 1s infinite":"none",color:listening?"white":t.textMuted }}>
              🎤
            </button>
...
export default function App() {
  ...
  const { lang, switchLang, t_, translating } = useLang();

  // Build theme — country base + dark/beige override
  const baseTheme = COUNTRY_THEMES[user?.country] || COUNTRY_THEMES["Other"];
  const t = themeMode === "dark"
    ? { ...baseTheme, ...DARK_THEME, accent: baseTheme.accent, accentText: baseTheme.accentText, accentSoft: baseTheme.accentSoft }
    : themeMode === "beige"
    ? { ...baseTheme, ...BEIGE_THEME, accent: baseTheme.accent, accentText: baseTheme.accentText, accentSoft: baseTheme.accentSoft }
    : baseTheme;
  ...
  return (
    <div style={{
      ...
      <Nav
        ...
        translating={translating} t_={t_}
        themeMode={themeMode} setThemeMode={setThemeMode}
      />

      <main>
        <HeroSection         user={user} t={t} t_={t_}/>
        <AboutSection        t={t}/>
