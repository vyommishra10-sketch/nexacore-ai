...
const INDUSTRY_SERVICES = {
  Technology:    [{ icon:"🧠", title:"AI Automation Suite",   desc:"End-to-end workflow automation powered by adaptive ML models." },
                  { icon:"📡", title:"DevOps Intelligence",    desc:"Predict failures, optimise pipelines, ship with confidence." },
                  { icon:"☁️", title:"Cloud Optimisation",     desc:"Right-size cloud spend with continuous AI recommendations." },
                  { icon:"📊", title:"Predictive Analytics",   desc:"Turn raw telemetry into forward-looking product insights." }],
  Healthcare:    [{ icon:"🏥", title:"Patient Data AI",        desc:"Unified patient intelligence with full HIPAA compliance." },
                  { icon:"📋", title:"Compliance Automation",  desc:"Stay audit-ready with real-time regulatory monitoring." },
                  { icon:"🔬", title:"Diagnostic Insights",    desc:"AI-assisted pattern recognition across imaging and lab data." },
                  { icon:"⚡", title:"Workflow Optimiser",     desc:"Streamline clinical workflows and reduce admin burden." }],
  Finance:       [{ icon:"📉", title:"Risk Intelligence",      desc:"Real-time portfolio risk scoring with explainable AI." },
                  { icon:"🛡️", title:"Fraud Detection AI",     desc:"Sub-millisecond transaction screening at 99.7% precision." },
                  { icon:"⚖️", title:"RegTech Automation",     desc:"Automated compliance reporting across 40+ jurisdictions." },
                  { icon:"💹", title:"Investment Insights",    desc:"Quantitative signals fused with macro and alternative data." }],
  Education:     [{ icon:"🎓", title:"Adaptive Learning AI",   desc:"Personalised learning paths that evolve with every student." },
                  { icon:"📈", title:"Student Analytics",      desc:"Early-warning signals for at-risk students across cohorts." },
                  { icon:"📚", title:"Curriculum Optimiser",   desc:"Data-driven curriculum design aligned to outcomes." },
                  { icon:"🖥️", title:"EdTech Platform",        desc:"White-label AI infrastructure for education providers." }],
  Retail:        [{ icon:"🔮", title:"Demand Forecasting",     desc:"SKU-level predictions with 93% accuracy up to 12 weeks out." },
                  { icon:"🛒", title:"Customer AI",            desc:"Hyper-personalised recommendations across every channel." },
                  { icon:"📦", title:"Inventory Intelligence", desc:"Eliminate stockouts and overstock with smart replenishment." },
                  { icon:"✨", title:"Personalisation Engine", desc:"1:1 customer experiences at the scale of millions." }],
  Manufacturing: [{ icon:"🔧", title:"Predictive Maintenance", desc:"Catch equipment failures before they cost you downtime." },
                  { icon:"🚛", title:"Supply Chain AI",        desc:"End-to-end visibility with disruption prediction." },
                  { icon:"✅", title:"Quality Control AI",     desc:"Computer-vision defect detection at line speed." },
                  { icon:"🏭", title:"Factory Analytics",      desc:"OEE dashboards with AI-driven improvement recommendations." }],
  Legal:         [{ icon:"📄", title:"Contract AI",            desc:"Extract, review and risk-score contracts in seconds." },
                  { icon:"🔍", title:"Compliance Monitor",     desc:"Continuous scanning across regulations and jurisdictions." },
                  { icon:"⚖️", title:"Legal Research AI",      desc:"Deep case-law search with citation graph analysis." },
                  { icon:"💼", title:"Case Intelligence",      desc:"Outcome prediction and strategy modelling for litigation." }],
  Marketing:     [{ icon:"🚀", title:"Campaign AI",            desc:"Self-optimising campaigns that improve with every impression." },
                  { icon:"👥", title:"Audience Intelligence",  desc:"Build precise segments from first and third-party signals." },
                  { icon:"✍️", title:"Content Optimiser",      desc:"AI-generated content aligned to brand voice." },
                  { icon:"📡", title:"Attribution Engine",     desc:"Multi-touch attribution across online and offline channels." }],
  Other:         [{ icon:"🧠", title:"Enterprise AI Core",     desc:"Foundational AI infrastructure built for your use case." },
                  { icon:"📊", title:"Data Intelligence",      desc:"Unified data layer with automated insight generation." },
                  { icon:"⚡", title:"Process Automation",     desc:"AI-driven automation for any repeatable business process." },
                  { icon:"🔮", title:"Smart Analytics",        desc:"Self-serve analytics for teams of all technical levels." }],
};
...
function Onboarding({ onComplete }) {
  ...
  const inp = {
    width:"100%", padding:"12px 15px", borderRadius:10, boxSizing:"border-box",
    background:t.surface2, border:`1.5px solid ${t.border}`,
    color:t.text, fontSize:15, outline:"none", fontFamily:"inherit", marginTop:6,
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:t.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 20px" }}>
      ...
      <div key={step} className="nc-fade" style={{ width:"100%",maxWidth:480,background:t.surface,borderRadius:20,border:`1px solid ${t.border}`,padding:"34px 34px 30px",boxShadow:`0 4px 40px ${t.accent}14` }}>
        ...
        {step===0 && (
          <><label style={{ color:t.textMuted,fontSize:13,fontWeight:600 }}>Full Name</label>
          <input autoFocus style={inp} placeholder="e.g. Divya Sharma" value={form.name}
            onChange={e=>setForm(f=>({...f,name:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&next()}/></>
        )}
        {step===1 && (
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
            {INDUSTRIES.map(ind=>(
              <button key={ind} onClick={()=>setForm(f=>({...f,industry:ind}))} style={{ padding:"10px 13px",borderRadius:8,cursor:"pointer",background:form.industry===ind?t.accent:t.surface2,border:`1.5px solid ${form.industry===ind?t.accent:t.border}`,color:form.industry===ind?t.accentText:t.text,fontSize:13,fontWeight:600,textAlign:"left",transition:"all 0.15s" }}>{ind}</button>
            ))}
          </div>
        )}
        {step===2 && (
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
            {COUNTRIES.map(c=>(
              <button key={c} onClick={()=>setForm(f=>({...f,country:c}))} style={{ padding:"10px 13px",borderRadius:8,cursor:"pointer",background:form.country===c?t.accent:t.surface2,border:`1.5px solid ${form.country===c?t.accent:t.border}`,color:form.country===c?t.accentText:t.text,fontSize:13,fontWeight:600,textAlign:"left",transition:"all 0.15s",display:"flex",gap:6,alignItems:"center" }}>
                <span>{COUNTRY_THEMES[c]?.flag}</span><span>{c}</span>
              </button>
            ))}
          </div>
        )}
        ...
        <div style={{ display:"flex",gap:10,marginTop:22 }}>
          {step>0 && (
            <button onClick={()=>setStep(s=>s-1)} style={{ padding:"11px 17px",borderRadius:10,border:`1.5px solid ${t.border}`,background:"transparent",color:t.textMuted,cursor:"pointer",fontSize:14,fontWeight:600 }}>← Back</button>
          )}
...
// ─── CONTACT PAGE ─────────────────────────────────────────────
function ContactPage({ user, t, onClose }) {
  ...
  return (
    <div className="nc-slide" style={{ position:"fixed",inset:0,zIndex:800,background:t.bg,overflowY:"auto",display:"flex",flexDirection:"column" }}>
      <div style={{ padding:"16px 32px",borderBottom:`1px solid ${t.border}`,background:t.surface,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10 }}>
        ...
        <button onClick={onClose} style={{ background:"transparent",border:`1px solid ${t.border}`,color:t.textMuted,padding:"7px 16px",borderRadius:8,cursor:"pointer",fontSize:14,fontWeight:600 }}>
          ← Back to site
        </button>
      </div>
      ...
      <div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"48px 24px" }}>
        {sent ? (
          <div className="nc-fade" style={{ textAlign:"center",maxWidth:420 }}>
            ...
            <p style={{ color:t.textMuted,fontSize:15,lineHeight:1.7,marginBottom:26 }}>
              Our team will reach out within 48 hours, {user?.name}. In the meantime, explore what NexaCore can do for you.
            </p>
            <button onClick={onClose} style={{ padding:"12px 28px",borderRadius:10,border:"none",background:t.accent,color:t.accentText,cursor:"pointer",fontSize:15,fontWeight:700,boxShadow:`0 6px 24px ${t.accent}40` }}>
              Explore NexaCore →
            </button>
          </div>
        ) : (
          <div style={{ width:"100%",maxWidth:520 }}>
            <div className="nc-fade" style={{ marginBottom:32,textAlign:"center" }}>
              <div style={{ display:"inline-block",background:t.accentSoft,color:t.accent,fontSize:11,fontWeight:700,padding:"4px 14px",borderRadius:100,marginBottom:12,letterSpacing:"0.08em",textTransform:"uppercase" }}>
                Get in Touch
              </div>
              <h1 style={{ fontSize:"clamp(1.7rem,4vw,2.4rem)",fontWeight:900,color:t.text,letterSpacing:"-0.8px",marginBottom:10 }}>
                Let's talk about your enterprise.
              </h1>
              <p style={{ color:t.textMuted,fontSize:15,lineHeight:1.7 }}>
                Tell us about your needs. We'll come back with a custom AI deployment plan — no commitment required.
              </p>
            </div>
            <div style={{ background:t.surface,border:`1px solid ${t.border}`,borderRadius:16,padding:"30px",display:"flex",flexDirection:"column",gap:15 }}>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
                <div>
                  <label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Your Name</label>
                  <input style={inp} placeholder="Jane Smith" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/>
                </div>
                <div>
                  <label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Work Email</label>
                  <input style={inp} type="email" placeholder="jane@company.com" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))}/>
                </div>
              </div>
              <div>
                <label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>Company</label>
                <input style={inp} placeholder="Acme Corp" value={form.company} onChange={e=>setForm(p=>({...p,company:e.target.value}))}/>
              </div>
              <div>
                <label style={{ color:t.textMuted,fontSize:13,fontWeight:600,display:"block",marginBottom:5 }}>What are you looking to solve?</label>
                <textarea style={{...inp,resize:"vertical"}} rows={4} placeholder="Tell us about your biggest operational challenge…" value={form.message} onChange={e=>setForm(p=>({...p,message:e.target.value}))}/>
              </div>
              <button onClick={()=>setSent(true)} style={{ padding:"13px",borderRadius:10,border:"none",cursor:"pointer",background:t.accent,color:t.accentText,fontWeight:700,fontSize:15,fontFamily:"inherit",boxShadow:`0 6px 24px ${t.accent}40` }}>
                Send Message →
              </button>
              <p style={{ color:t.textMuted,fontSize:12,textAlign:"center" }}>🔒 We never share your data. No spam, ever.</p>
            </div>
          </div>
        )}
      </div>
...
function ContactFAB({ t, onClick }) {
  ...
  return (
    <div style={{ position:"relative",display:"inline-flex",alignItems:"center",justifyContent:"center" }}>
      {[0,1].map(i=>(
        <div key={i} style={{ position:"absolute",width:"100%",height:"100%",borderRadius:"50%",background:t.accent,animation:`ripple 2s ease-out ${i*0.9}s infinite`,pointerEvents:"none" }}/>
      ))}
      <button onClick={onClick}
        onMouseEnter={()=>setHov(true)}
        onMouseLeave={()=>setHov(false)}
        style={{ "--ring":`${r},${g},${b}`,position:"relative",width:52,height:52,borderRadius:"50%",border:"none",background:t.accent,color:t.accentText,fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",animation:"blink 2.2s ease-in-out infinite",boxShadow:`0 4px 20px ${t.accent}55`,transition:"transform 0.2s",transform:hov?"scale(1.1)":"scale(1)",zIndex:1 }}>
        ✉️
      </button>
      {hov && (
        <div style={{ position:"absolute",right:62,top:"50%",transform:"translateY(-50%)",background:t.text,color:t.bg,padding:"5px 11px",borderRadius:7,fontSize:12,fontWeight:600,whiteSpace:"nowrap",pointerEvents:"none",zIndex:2 }}>
          Get in Touch
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ background:t.accentSoft,borderBottom:`1px solid ${t.border}`,padding:"8px 32px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8 }}>
      <span style={{ color:t.textMuted,fontSize:13 }}>
        {th.flag} Welcome back, <strong style={{color:t.text}}>{user.name}</strong> · Last visited {last}
      </span>
  ...
  return (
    <nav style={{ position:"sticky",top:0,zIndex:500,background:t.surface+"EE",backdropFilter:"blur(12px)",borderBottom:`1px solid ${t.border}`,padding:"0 32px",height:56,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
      ...
      <div style={{ display:"flex",gap:26 }}>
        {[["About","#about"],["Capabilities","#capabilities"],["Solutions","#solutions"]].map(([label,href])=>(
      ...
      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
        ...
        <button onClick={onContactClick} style={{ background:t.accent,color:t.accentText,padding:"7px 16px",borderRadius:8,border:"none",fontSize:13,fontWeight:700,cursor:"pointer",boxShadow:`0 4px 14px ${t.accent}44` }}>
          Get in Touch
        </button>
      </div>
    </nav>
  ...
  return (
    <section style={{ padding:"56px 40px 48px",textAlign:"center",background:t.bg }}>
      ...
      <div style={{ display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap" }}>
        <a href="#solutions" style={{ background:t.accent,color:t.accentText,padding:"12px 26px",borderRadius:10,textDecoration:"none",fontWeight:700,fontSize:14,boxShadow:`0 6px 22px ${t.accent}40` }}>
          View My Recommendations →
        </a>
        <a href="#capabilities" style={{ background:"transparent",border:`1.5px solid ${t.border}`,color:t.text,padding:"12px 26px",borderRadius:10,textDecoration:"none",fontWeight:600,fontSize:14 }}>
    ...
    <section id="about" style={{ padding:"60px 40px",background:t.surface2,borderTop:`1px solid ${t.border}` }}>
      <div style={{ maxWidth:1020,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center" }}>
        <div>
          ...
          <h2 style={{ fontSize:"clamp(1.4rem,2.6vw,2rem)",fontWeight:900,color:t.text,marginBottom:14,lineHeight:1.2,letterSpacing:"-0.4px" }}>
            AI that thinks in your industry's language.
          </h2>
          <p style={{ color:t.textMuted,fontSize:14,lineHeight:1.8,marginBottom:12 }}>
            NexaCore was built because generic AI tools weren't enough. Every enterprise has unique compliance requirements, workflows, and market pressures.
          </p>
          <p style={{ color:t.textMuted,fontSize:14,lineHeight:1.8 }}>
            We built a platform that learns your context from the moment you arrive and delivers intelligence that actually fits your world.
          </p>
          <div style={{ marginTop:22,display:"flex",gap:8,flexWrap:"wrap" }}>
            {["ISO 27001","GDPR","SOC 2 Type II"].map(b=>(
              <span key={b} style={{ background:t.surface,border:`1px solid ${t.border}`,color:t.textMuted,fontSize:12,fontWeight:600,padding:"5px 12px",borderRadius:100 }}>{b}</span>
            ))}
          </div>
        </div>
    ...
    <section id="capabilities" style={{ padding:"60px 40px",background:t.bg,borderTop:`1px solid ${t.border}` }}>
      <div style={{ maxWidth:1020,margin:"0 auto" }}>
        <div style={{ textAlign:"center",marginBottom:36 }}>
          ...
          <p style={{ color:t.textMuted,maxWidth:400,margin:"0 auto",fontSize:14,lineHeight:1.7 }}>From intelligent automation to global compliance — we handle the complexity.</p>
        </div>
      ...
      <div style={{ maxWidth:1020,margin:"0 auto" }}>
        ...
        <div style={{ background:t.accentSoft,border:`1px solid ${t.accent}33`,borderRadius:12,padding:"16px 20px",marginBottom:32,display:"flex",alignItems:"flex-start",gap:12 }}>
          ...
          <div>
            ...
            {profLoad
              ? <div style={{ color:t.textMuted,fontSize:14,animation:"pulse 1s infinite" }}>Analysing your profile…</div>
        ...
        <div style={{ textAlign:"center",marginBottom:28 }}>
          <div style={{ display:"inline-block",background:t.accentSoft,color:t.accent,fontSize:11,fontWeight:700,padding:"4px 13px",borderRadius:100,marginBottom:12,letterSpacing:"0.08em",textTransform:"uppercase" }}>
            Recommended for {industry}
          </div>
          <h2 style={{ fontSize:"clamp(1.4rem,2.6vw,2rem)",fontWeight:900,color:t.text,marginBottom:8,letterSpacing:"-0.4px" }}>
            Your personalised AI stack, {user.name}.
          </h2>
          <div style={{ position:"relative",maxWidth:420,margin:"14px auto 0" }}>
            ...
            <input value={query} onChange={e=>setQuery(e.target.value)}
              placeholder="Search services (e.g. 'fraud detection', 'AI automation')…"
            ...
            {searching && <span style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",animation:"pulse 1s infinite",fontSize:13 }}>⏳</span>}
            {query && !searching && (
              <button onClick={()=>{setQuery("");setAiResult(null);}} style={{ position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:t.textMuted,cursor:"pointer",fontSize:15 }}>✕</button>
            )}
          </div>
          {aiResult?.reason && (
            <div style={{ display:"inline-flex",alignItems:"center",gap:6,marginTop:8,background:t.accentSoft,border:`1px solid ${t.accent}33`,borderRadius:100,padding:"3px 13px",fontSize:12,color:t.accent,fontWeight:600 }}>
              ✦ {aiResult.reason}
            </div>
          )}
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:14 }}>
          {displayed.map((svc,i)=>(
            <div key={svc.title} className="nc-fade" style={{ animationDelay:`${i*0.055}s`,background:t.surface,border:`1.5px solid ${i===0&&!aiResult?t.accent:t.border}`,borderRadius:13,padding:"22px 18px",position:"relative",overflow:"hidden",transition:"transform 0.2s,box-shadow 0.2s" }}
              ...
              {i===0&&!aiResult&&<div style={{ position:"absolute",top:10,right:10,background:t.accent,color:t.accentText,fontSize:9,fontWeight:800,padding:"2px 8px",borderRadius:100 }}>TOP PICK</div>}
...
function Chatbot({ user, t }) {
  ...
  const SYSTEM = `You are NexaCore AI's enterprise assistant. User: Name=${user.name}, Industry=${user.industry}, Country=${user.country}. NexaCore is an enterprise AI platform. Reply in 2-4 sentences max. Be professional and relate to user's industry. Never say you are Claude or Anthropic.`;
  ...
  const quick = [`${user.industry} solutions`, "How does personalisation work?", "Is there a free trial?", "Book a demo"];

  return (
    <>
      <button onClick={()=>setOpen(o=>!o)}
        style={{ position:"fixed",bottom:24,right:88,zIndex:900,width:50,height:50,borderRadius:"50%",border:"none",background:t.accent,color:t.accentText,fontSize:20,cursor:"pointer",boxShadow:`0 8px 28px ${t.accent}55`,display:"flex",alignItems:"center",justifyContent:"center",transition:"transform 0.2s" }}
      ...
      {open && (
        <div className="nc-slide" style={{ position:"fixed",bottom:84,right:88,zIndex:900,width:320,maxHeight:490,background:t.surface,borderRadius:16,border:`1px solid ${t.border}`,boxShadow:`0 16px 56px rgba(0,0,0,.18)`,display:"flex",flexDirection:"column",overflow:"hidden" }}>
          <div style={{ padding:"12px 15px",background:t.accent,display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
            ...
            <div>
              <div style={{ color:t.accentText,fontWeight:700,fontSize:14 }}>NexaCore Assistant</div>
              <div style={{ color:`${t.accentText}BB`,fontSize:11 }}>AI-powered · Always available</div>
            </div>
          </div>
          <div style={{ flex:1,overflowY:"auto",padding:12,display:"flex",flexDirection:"column",gap:8 }}>
            {msgs.map((m,i)=>(
              <div key={i} style={{ alignSelf:m.role==="user"?"flex-end":"flex-start",maxWidth:"85%",background:m.role==="user"?t.accent:t.surface2,color:m.role==="user"?t.accentText:t.text,border:m.role==="bot"?`1px solid ${t.border}`:"none",borderRadius:m.role==="user"?"12px 12px 2px 12px":"12px 12px 12px 2px",padding:"9px 12px",fontSize:13,lineHeight:1.55 }}>
                {m.text}
              </div>
            ))}
            {loading && (
              <div style={{ alignSelf:"flex-start",background:t.surface2,border:`1px solid ${t.border}`,borderRadius:"12px 12px 12px 2px",padding:"9px 13px",color:t.textMuted,fontSize:13 }}>
                <span style={{animation:"pulse 1s infinite"}}>● ● ●</span>
              </div>
            )}
            <div ref={bottomRef}/>
          </div>
          <div style={{ padding:"6px 10px",borderTop:`1px solid ${t.border}`,display:"flex",gap:5,flexWrap:"wrap",flexShrink:0 }}>
            {quick.map(q=>(
              <button key={q} onClick={()=>send(q)} style={{ background:t.surface2,border:`1px solid ${t.border}`,color:t.textMuted,borderRadius:20,padding:"4px 9px",fontSize:11,cursor:"pointer" }}>{q}</button>
            ))}
          </div>
          <div style={{ padding:"9px 10px",borderTop:`1px solid ${t.border}`,display:"flex",gap:7,flexShrink:0 }}>
            ...
            <button onClick={()=>send()} disabled={loading||!input.trim()}
              style={{ background:loading||!input.trim()?t.border:t.accent,border:"none",borderRadius:8,padding:"8px 13px",color:loading||!input.trim()?t.textMuted:t.accentText,cursor:loading||!input.trim()?"default":"pointer",fontSize:15,transition:"background 0.2s" }}>→</button>
          </div>
        </div>
      )}
    </>
...
function Footer({ t }) {
  return (
    <footer style={{ padding:"22px 40px",borderTop:`1px solid ${t.border}`,background:t.surface,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12 }}>
      <div style={{ display:"flex",alignItems:"center",gap:8 }}>
        <div style={{ width:22,height:22,borderRadius:6,background:t.accent,display:"flex",alignItems:"center",justifyContent:"center",color:t.accentText,fontWeight:900,fontSize:11 }}>N</div>
        <span style={{ color:t.textMuted,fontSize:13 }}>© 2026 NexaCore AI. All rights reserved.</span>
      </div>
      <div style={{ display:"flex",gap:20 }}>
        {["Privacy","Terms","Security","Docs"].map(l=>(
          <a key={l} href="#" style={{ color:t.textMuted,fontSize:13,textDecoration:"none" }}>{l}</a>
        ))}
      </div>
    </footer>
  );
...
export default function App() {
  ...
  return (
    <div style={{ background:t.bg, color:t.text, minHeight:"100vh" }}>
      {showContact && <ContactPage user={user} t={t} onClose={()=>setShowContact(false)}/>}
